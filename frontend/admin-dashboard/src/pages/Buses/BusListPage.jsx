    ...
    useEffect(() => {
        const fetchBuses = async () => {
            try {
                ...
                // Giả sử API trả về mảng xe buýt
                setBuses(response.data); 
            } catch (err) {
                console.error("Failed to fetch buses:", err);
                const errorMessage = err.response?.data?.message || 'Không thể tải dữ liệu xe buýt.';
    ...
    return (
        <div className="bus-list-container">
            <header style={styles.header}>
                ...
                {/* Nút Thêm mới */}
                <button style={styles.addButton}>+ Thêm Xe buýt mới</button>
            </header>
                    ...
                    <tbody>
                        {buses.map((bus) => (
                            <tr key={bus.id} style={styles.tr}>
                                ...
                                <td style={styles.td}>
                                    {bus.Route ? bus.Route.name : 'N/A'}
                                </td>
                                <td style={styles.tdAction}>
                                    <button style={styles.actionButton('edit')}>Sửa</button>
                                    <button style={styles.actionButton('detail')}>Chi tiết</button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
