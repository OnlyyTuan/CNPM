...
// Sử dụng thư viện icon (ví dụ: react-icons)
...
const StudentListPage = () => {
    ...
    useEffect(() => {
        const fetchStudents = async () => {
            try {
                ...
                // Giả sử API trả về mảng học sinh trực tiếp
                setStudents(response.data); 
            } catch (err) {
                console.error("Failed to fetch students:", err);
                // Xử lý lỗi (ví dụ: 403 Forbidden nếu role không đủ, 401 nếu token hết hạn)
                const errorMessage = err.response?.data?.message || 'Không thể tải dữ liệu học sinh.';
    ...
    if (error) {
        return (
            <div className="container" style={{ color: 'red', textAlign: 'center', marginTop: '50px' }}>
                <h2>Lỗi!</h2>
                <p>{error}</p>
            </div>
        );
    ...
    return (
        <div className="student-list-container">
            <header style={styles.header}>
                ...
                {/* Nút Thêm mới */}
                <button style={styles.addButton}>+ Thêm Học sinh mới</button>
            </header>
                    ...
                    <tbody>
                        {students.map((student) => (
                            <tr key={student.id} style={styles.tr}>
                                ...
                                <td style={styles.td}>{student.name || student.fullName}</td> {/* Dùng name hoặc fullName */}
                                <td style={styles.td}>{student.className}</td>
                                <td style={styles.td}>
                                    <span style={styles.statusBadge(student.status)}>{student.status}</span>
                                </td>
                                ...
                                */}
                                <td style={styles.td}>
                                    {student.AssignedBus ? student.AssignedBus.license_plate : 'Chưa gán'}
                                </td>
                                <td style={styles.td}>
