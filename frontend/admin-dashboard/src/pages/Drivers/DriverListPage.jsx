    ...
    useEffect(() => {
        const fetchDrivers = async () => {
            try {
                ...
                // Giả sử API trả về mảng tài xế (kèm theo thông tin Bus)
                setDrivers(response.data); 
            } catch (err) {
                console.error("Failed to fetch drivers:", err);
                const errorMessage = err.response?.data?.message || 'Không thể tải dữ liệu tài xế.';
    ...
    return (
        <div className="driver-list-container">
            <header style={styles.header}>
                ...
                {/* Nút Thêm mới */}
                <button style={styles.addButton}>+ Thêm Tài xế mới</button>
            </header>
                    ...
                    <tbody>
                        {drivers.map((driver) => (
                            <tr key={driver.id} style={styles.tr}>
                                ...
                                <td style={styles.td}>
                                    {driver.CurrentBus ? driver.CurrentBus.license_plate : 'OFF_DUTY'}
                                </td>
