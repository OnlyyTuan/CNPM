    ...
    try {
      ...
      if (response.success && response.data && response.data.token) {
         ...
         if (role !== 'parent' && role !== 'admin') {
             toast.error('Tài khoản không phải là phụ huynh');
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-400 via-pink-500 to-pink-600 flex items-center justify-center p-4">
      <div className="max-w-md w-full">
        ...
        <div className="text-center mb-8">
          <div className="flex justify-center mb-4">
            <div className="bg-white p-4 rounded-full shadow-lg">
              <Bus size={48} className="text-pink-600" />
            </div>
          </div>
          <h1 className="text-4xl font-bold text-white mb-2">
            SmartBus Parent
          </h1>
          <p className="text-pink-100">Cổng thông tin điện tử Phụ huynh</p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-2xl p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button
              type="submit"
              disabled={loading}
              className={`w-full py-3 px-4 bg-pink-600 text-white rounded-lg font-semibold
                ${
                  loading
          ...
          <div className="mt-6 p-4 bg-pink-50 rounded-lg border border-pink-200">
            <p className="text-sm text-pink-800 font-medium mb-2">
              📝 Thông tin đăng nhập Phụ huynh:
            </p>
            <div className="text-sm text-pink-700 space-y-1">
              <p>
                • Tên đăng nhập:{" "}
                <span className="font-mono font-bold">parent1</span>
              </p>
              <p>
        ...
        {/* Footer */}
        <p className="text-center text-pink-100 text-sm mt-6">
