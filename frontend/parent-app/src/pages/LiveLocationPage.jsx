  ...
  useEffect(() => {
    const loadRoutes = async () => {
      try {
        ...
        const routeColors = ['#3b82f6', '#10b981', '#9333ea', '#ef4444', '#8b5cf6'];
        const routeData = {};
      ...
      try {
        ...
        if (!canceled) {
          const cleaned = Array.isArray(data)
            ? data.filter((x) => x && x.lat != null && x.lng != null && x.status !== 'INACTIVE' && x.status !== 'MAINTENANCE')
            : [];

          const withTimestamp = cleaned.map(bus => ({ ...bus, _fetchTime: Date.now() }));
      ...
      <div className="bg-white rounded-xl shadow-md p-6">
        ...
        <div className="rounded-lg overflow-hidden" style={{ height: '500px' }}>
          ...
          >
            ...
            {stops.map((stop) => {
              const stopIcon = L.divIcon({ className: '', html: '<div style="font-size:16px;">🚏</div>', iconSize: [16, 16], iconAnchor: [8, 16], popupAnchor: [0, -16] });
              return (
                <Marker
                  key={stop.id}
